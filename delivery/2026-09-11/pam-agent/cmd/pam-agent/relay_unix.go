...
func cmdRelay(args []string) error {
	...
	case result.ExitCode != 0:
		// STATUS STAYS COMPLETED. The tool started, the operator used it, and
		// the recording is the evidence of what they did; the exit status is
		// context on that session, not a verdict that the launch failed.
		//
		// This case used to set FAILED, and on a real deployment it made every
		// working session look like a failure: psql, sqlplus, mongosh and
		// redis-cli all return non-zero when the operator closes the window.
		// One Oracle session in that log ran fourteen hours and captured four
		// commands, and was still filed as FAILED.
		failure = reportEndFailure(result.ExitCode)
	}
