...
// SQL Developer FIRST, then the two command-line tools, then the browser.
//
// This is Oracle's own answer to "how do people work with this database", and
// it is deliberately the opposite of the MongoDB ordering. An Oracle operator
// who has SQL Developer installed installed it on purpose: it is the tool
// Oracle ships, the one their runbooks are written against, and the one that
// makes a schema browsable rather than something you type SELECT statements
// at. Ordered by what the operator would pick, not by what is cheapest for
// the agent to drive.
//
// sqlplus and sqlcl stay ahead of the browser rather than being removed: a
// server, a container or a locked-down laptop often has a client and no
// desktop app at all, and the browser is last because it is the only one
// selected without any discovery and the only one the agent cannot record.
func TestDefaultTemplatesOracleOrder(t *testing.T) {
	got := make([]string, 0, 4)
	for _, c := range defaultCandidates(t, "oracle") {
		got = append(got, c.ID+"/"+c.Kind)
	}
	want := "sqldeveloper/gui,sqlplus/cli,sqlcl/cli,oracle-web/browser"
