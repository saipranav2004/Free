...
func Spawn(pc *PreparedCommand, rec *recorder.Cast) (waited bool, enforcement Summary, err error) {
	...
	for _, cand := range candidates {
		...
		c.Env = os.Environ()
		err = c.Run()
		// A non-zero status here is the TOOL's, carried out through the
		// terminal emulator, and it is not a failed launch: the window opened,
		// the operator used it, and the wrapper inside it has already reported
		// the session end. Returning a bare error made the caller report the
		// session FAILED a second time, overwriting the wrapper's COMPLETED.
		// See launcher.ToolExitError.
		if code, ok := exitStatusOf(err); ok {
			return cand.blocks, enforcement, &ToolExitError{Exec: pc.Exec, Code: code}
		}
