...
func spawnCLIWithConPTY(pc *PreparedCommand, rec *recorder.Cast) (waited bool, enforcement Summary, err error) {
	...
	if exitCode != 0 {
		// A TYPED error, not a bare one: the caller has to be able to tell
		// this apart from "the tool would not start". An interactive client
		// exits non-zero every time the operator closes its window, and
		// reporting that as a failed launch filed every working session on
		// this machine as a failure. See ToolExitError.
		return true, enforcement, &ToolExitError{Exec: pc.Exec, Code: int(exitCode)}
	}
