...
function SelfServiceOnly({ to, children }) {
  ...
  // The QUERY TRAVELS WITH THE REDIRECT. Both pages spell their tabs the same
  // way (?tab=requests, ?tab=grants), so an admin who follows a link to
  // /jit?tab=grants should arrive on the approvals page's Grants tab, not be
  // dropped on Requests because a redirect ate the part of the URL that said
  // where they were going.
  return isAdmin ? <Navigate to={`${to}${search}`} replace /> : children
}
