...
// ---------------------------------------------------------------------------
// Server-computed equivalents
// ---------------------------------------------------------------------------
// The four functions above walk an array of events the browser had to download
// first, which is why the sample was capped at 5,000. GET /admin/audit/stats
// does the same arithmetic in the database over EVERY event in range and
// returns a few hundred bytes.
//
// These adapters exist so the chart components never learn which source they
// are being fed from: each returns exactly the shape its row-walking twin
// returns. One set of components, two ways to fill them, no branching in the
// rendering code.

/**
 * Server buckets to the fixed-width slots the volume chart draws.
 *
 * The server returns one row per bucket that actually had events, keyed by an
 * absolute timestamp. The chart wants a dense run of N slots ending now, with
 * empty ones present so the axis is continuous. This lays the sparse rows onto
 * that grid using the same edges bucketByTime computes, so switching source
 * cannot shift a column by an hour.
 */
export function bucketsFromStats(statBuckets, { buckets = 24, span = 'hour' } = {}) {
  const width = span === 'day' ? DAY : HOUR
  const now = Date.now()
  const end =
    span === 'day'
  ...
  for (const b of statBuckets || []) {
    const t = new Date(b?.start || 0).getTime()
    if (Number.isNaN(t) || t < start || t > end) continue
    const idx = Math.min(buckets - 1, Math.floor((t - start) / width))
    if (idx >= 0) {
      slots[idx].value += Number(b.count) || 0
      slots[idx].denied += Number(b.denied) || 0
    }
  }
  return slots
}

// The outcomes the console counts as "denied or failed". Mirrors the server's
// own list in audit_service.go; both exist because the two halves must agree on
// what a failure is or the chart and the tile disagree.
const FAILURE_OUTCOMES = ['DENIED', 'ERROR', 'FAILURE']

/** attentionCounts, from server totals. */
export function countsFromStats(stats) {
  const outcomes = stats?.outcomes || {}
  let failed = 0
  for (const k of FAILURE_OUTCOMES) failed += Number(outcomes[k]) || 0
  return {
...
/** outcomeBreakdown, from server totals. */
export function outcomeFromStats(stats) {
  const outcomes = stats?.outcomes || {}
  let success = 0
  let denied = 0
  let other = 0
  for (const [key, n] of Object.entries(outcomes)) {
    const v = Number(n) || 0
    const o = String(key).toUpperCase()
    if (o === 'SUCCESS' || o === 'ALLOWED') success += v
    else if (FAILURE_OUTCOMES.includes(o)) denied += v
    else other += v
  }
  return { success, denied, other, total: success + denied + other }
}

/**
 * offHoursShare, from the server's weekday/hour grid.
 *
 * Uses the SAME window the row-walking version uses (Mon to Fri, 07:00 to
 * 19:00), and the server already cut the grid in the caller's time zone, so
 * "outside hours" means the same thing either way.
 */
export function offHoursFromStats(stats) {
  let off = 0
  let counted = 0
  for (const cell of stats?.heat || []) {
    ...
    counted += n
    if (day >= 5 || hour < 7 || hour >= 19) off += n
  }
  return { off, counted, pct: counted ? (off / counted) * 100 : 0 }
}

/** heatmapCells, from the server grid. Already the right shape; normalised. */
export function heatFromStats(stats) {
  return (stats?.heat || [])
    .map((c) => ({ day: Number(c?.day), hour: Number(c?.hour), count: Number(c?.count) || 0 }))
    .filter((c) => !Number.isNaN(c.day) && !Number.isNaN(c.hour))
}
...
export function postureFindings({ users, criticality, stats, grants } = {}) {
  ...
  if (bg > 0) {
    out.push({
      ...
      count: bg,
      // The break-glass QUEUE, not the page's default Requests tab. An action
      // called "Open break-glass" that lands somewhere else is worse than no
      // link: it reads as a broken product at the exact moment somebody is
      // chasing a live emergency elevation.
      to: '/admin/jit?tab=breakglass',
