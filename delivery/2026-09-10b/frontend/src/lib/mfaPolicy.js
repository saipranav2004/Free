import { readMfaStatus } from './mfaStatus'
...
// Distinguishes "the server said false" from "the server said nothing", which
// boolish above deliberately does not: it exists to read fields whose absence
// means false. Enrolment is the opposite case, absence means unknown, and
// treating that as false is what produced the permanent banner.
function firstBooleanOf(obj, keys) {
  for (const k of keys) {
    const v = obj?.[k]
...
export function readMfaPolicyPosture(me) {
  ...
  // THE NAMES THIS READS HAVE TO BE THE NAMES THE SERVER SENDS.
  //
  // Three of them were not. The API spells it `mfa_enrolment_required`, one
  // "l", and this read `mfa_enrollment_required`; it sends the gating roles as
  // `mfa_policy_roles` and this looked for `mfa_required_by_roles`; it sends
  // the grace deadline as `mfa_grace_until` and this looked for
  // `mfa_enrollment_deadline`. So even on a response that carried the whole
  // decision, `blocked` read false, the roles came back empty and the
  // countdown never appeared.
  //
  // Both spellings are accepted rather than one being picked, because the
  // login response and /auth/me are two payloads built in two places and a
  // reader that tolerates both cannot be broken by whichever changes first.
  const enrolmentRequired = me.mfa_enrolment_required ?? me.mfa_enrollment_required
  const hasPolicyFields =
    'mfa_required' in me ||
    'mfa_policy_mode' in me ||
    'mfa_enrolment_required' in me ||
    'mfa_enrollment_required' in me

  const required = boolish(me.mfa_required)
  const blocked = boolish(enrolmentRequired)
  const mode = String(me.mfa_policy_mode || (required ? 'monitor' : 'off')).toLowerCase()

  // ENROLMENT IS THE ONE FIELD THIS CANNOT AFFORD TO READ NARROWLY.
  //
  // This used to be `boolish(me.mfa_enabled)` and nothing else, and the API
  // did not send that field at all. So `enrolled` was false for every account,
  // including accounts that had enrolled minutes earlier, and the banner below
  // asked them to set up a second factor they already had, on every page, for
  // ever. The server now reports mfa_enabled (see MFAPosture.Enrolled), which
  // is the answer to prefer: it is read from the device table, per request.
  //
  // lib/mfaStatus is the fallback rather than a second opinion. It knows the
  // other three signals the console can see, an authenticator in a device
  // list, the login flow's own evidence, and an enrolment completed in this
  // console, so a deployment still running the older API stops nagging as soon
  // as someone finishes enrolling here. It is consulted only when the explicit
  // field is absent, so it can never contradict the server.
  const explicitEnrolled = firstBooleanOf(me, ['mfa_enabled', 'mfaEnabled'])
  const enrolled = explicitEnrolled ?? readMfaStatus(me).enabled
  ...
  return {
    ...
    requiredByRoles: firstArray(
      me.mfa_policy_roles,
      me.mfaPolicyRoles,
      me.mfa_required_by_roles,
      me.mfaRequiredByRoles
    ),
    deadline:
      me.mfa_grace_until ||
      me.mfaGraceUntil ||
      me.mfa_enrollment_deadline ||
      me.mfaEnrollmentDeadline ||
      null,
  }
}
