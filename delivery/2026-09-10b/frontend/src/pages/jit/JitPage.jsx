...
export default function JitPage() {
  ...
  // In the URL, so the dashboard's "Active grants" and "Expiring soon" tiles
  // land on Grants rather than on Requests. See hooks/useUrlTab.js.
  const [tab, setTab] = useUrlTab(['requests', 'grants'], 'requests')
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // A JIT state change now also delivers notifications server-side, so the
      // bell and the notifications page are stale the moment this succeeds.
      // Without this the badge is wrong until its own 30 second poll comes
      // round, which is exactly long enough for somebody to notice.
