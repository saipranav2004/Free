...
function RequestsTab() {
  ...
  // ?queue= as well as ?tab=, because the tab alone cannot answer "show me
  // the ones that need a second approval": that is a facet inside Requests,
  // and it is what the overview's "Needs 2nd approval" tile counts.
  const [queueFilter, setQueueFilter] = useUrlTab(['all', 'mine', 'second'], 'all', 'queue')
  ...
  const invalidate = () => {
    ...
    // The bell reads its own queries on a 60s poll. Without this an approver
    // watched the request leave the queue while the badge kept counting it,
    // for up to a minute, and only a page refresh agreed with what they had
    // just done.
...
export default function AdminJitPage() {
  // In the URL, so the dashboard's "Break-glass active" and "Active grants"
  // tiles can land on the queue whose number was clicked instead of dropping
  // everyone on Requests. See hooks/useUrlTab.js.
