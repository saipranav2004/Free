...
func main() {
	...
	// pam_network_allowlist decides which source addresses may reach this
	// API at all. Migrated unconditionally, outside the dev-only block
	// above, for the same reason pam_jit_approvals is: production skips that
	// block entirely, and a security control that silently does not exist
	// there is worse than one that is switched off, because the console
	// would show an allowlist the API never consults.
	if err := db.AutoMigrate(&models.NetworkAllowlistEntry{}, &models.NetworkAllowlistSettings{}); err != nil {
		logger.Fatal("network_allowlist.migrate.fail",
			zap.Error(err),
			zap.String("impact", "the source-address allowlist cannot be enforced without pam_network_allowlist"))
	}
	networkAllowlistService := services.NewNetworkAllowlistService(db, cfg.Network.TrustedProxyState())
	...
	networkAllowlistHandler := handlers.NewNetworkAllowlistHandler(networkAllowlistService)
	...
	// that address, which forged every SourceIP written to the audit trail. nil
	// (the "none" sentinel, or no proxies configured) means trust nothing and
	// use the socket peer, which is the correct behaviour when this process is
	// reached directly, e.g. behind an ALB that is not yet named here.
	...
	// Source-address allowlist. After CORS so a browser preflight is answered
	// with its headers first and a blocked operator sees a real 403 rather
	// than an unexplained CORS failure; before every route group so no
	// authenticated path can be reached from a network root has not
	// permitted. /api/health is exempt so a blocked network cannot make the
	// orchestrator restart a container that is serving the permitted ones
	// perfectly well.
	r.Use(middleware.NetworkAllowlist(networkAllowlistService, []string{"/api/health"}))
	...
	// ── Allowlist as nginx directives, for the console container ──
	// Fetched by an entrypoint script with no operator and no JWT, so it
	// carries a shared token of its own rather than sitting behind the
	// normal auth. Registered only when that token is configured: the
	// response names every network permitted to reach the console, so an
	// unset token has to mean "this route does not exist" and not "anyone
	// may read it".
	//
	// Exempt from the allowlist middleware by necessity — the container
	// fetching it may well be on a network the list does not cover, and it
	// is fetching the very thing that would say so.
	if token := strings.TrimSpace(cfg.Network.SnippetToken); token != "" {
		r.GET("/api/v1/pam/network/allowlist.conf", func(c *gin.Context) {
			if subtle.ConstantTimeCompare([]byte(bearerToken(c)), []byte(token)) != 1 {
				c.Status(http.StatusNotFound)
				return
			}
			networkAllowlistHandler.NginxSnippet(c)
		})
	}
	...
	res.Use(middleware.EnrolmentOnlyGate())
	{
		...
		// How did that launch go? Everything after the pam-agent:// handoff
		// happens on the operator's own machine, so without this the console
		// cannot tell "psql opened" from "psql is not installed" from "no
		// agent is running here" — it showed the same nothing for all three.
		//
		// No permission gate beyond PAMAuth on purpose: the service scopes
		// every read to the launch's own owner, so the only thing a caller
		// can learn is how their own launch went. A resource permission check
		// here would be the wrong question anyway, since a launch may outlive
		// the grant that authorised it and the operator still needs to be
		// told why it failed.
		res.GET("/agent/launch/:launch_id/status", agentHandler.LaunchStatus)
	...
	admin.Use(middleware.ResolveDelegationScope(identityService.DelegationScopeFor, logger))
	{
		...
		// A desktop application session is an MP4, not a gzip'd transcript,
		// and a <video> element seeks with byte-range requests. Both are why
		// this is its own route rather than a branch of /cast.
		admin.GET("/recordings/:id/video", adminHandler.GetRecordingVideo)
		...
		// ── NETWORK (root only) ──
		// RequireRoot on top of the group's RequireAdmin: this is the control
		// that decides who can reach the product at all, so an admin able to
		// edit it could lock root out or open the console to a range root
		// never sanctioned.
		network := admin.Group("/network", middleware.RequireRoot())
		{
			network.GET("/allowlist", networkAllowlistHandler.Get)
			network.POST("/allowlist", networkAllowlistHandler.Add)
			network.DELETE("/allowlist/:id", networkAllowlistHandler.Remove)
			network.PUT("/allowlist/enabled", networkAllowlistHandler.SetEnabled)
		}
...
// tolerating the "Bearer " prefix being present or absent so the fetch
// script can send either.
