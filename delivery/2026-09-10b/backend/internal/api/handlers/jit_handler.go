...
func (h *JITHandler) Breakglass(c *gin.Context) {
	...
	h.notifyApprovers(services.NotifyInput{
		...
		// Straight to the break-glass queue. This notification asks the reader
		// to revoke something before a waiting period runs out, so the seconds
		// spent finding the right tab are seconds that matter.
		Link:       "/admin/jit?tab=breakglass",
		EntityType: "jit_request",
		EntityID:   created.ID,
...
func (h *JITHandler) Approve(c *gin.Context) {
	...
	if grant == nil {
		...
		h.notifyApprovers(services.NotifyInput{
			...
			// The facet too: this is precisely the "Needs 2nd approval" queue,
			// and it is the cheapest work an approver has, one click from live
			// access. Landing on the whole queue buries it.
			Link:       "/admin/jit?tab=requests&queue=second",
			EntityType: "jit_request",
			EntityID:   req.ID,
	...
	h.notifyUser(services.NotifyInput{
		...
		// Approval produced a GRANT, and the grant is what the reader wants to
		// see and use; the request it came from is finished business.
		Link:       "/jit?tab=grants",
		EntityType: "jit_request",
		EntityID:   req.ID,
