...
type MFAPosture struct {
	...
	// Enrolled is whether the account holds an ACTIVE second factor right now.
	//
	// It has to be here because the console cannot work it out. The device
	// table is not something a browser can read, and the other three fields
	// answer a different question: whether the account is REQUIRED to have a
	// factor, not whether it has one. Without this the console had no honest
	// way to tell "required and missing" from "required and satisfied", so it
	// asked everyone under a policy to enrol, including the people who already
	// had. The value is computed on every request anyway to decide
	// EnrolmentRequired; this only stops it being thrown away.
...
func (h *AuthHandler) Me(c *gin.Context) {
	...
	if h.posture != nil {
		...
		if p, err := h.posture(uid, roles); err != nil {
			...
			// Stated, never implied. See MFAPosture.Enrolled.
