...
func (s *JITService) ExpireDueGrants(ctx context.Context) (expired int, sessionsKilled int, errCount int) {
	...
	for i := range due {
		...
		s.tell(NotifyInput{
			...
			// The tab is part of the link. A notification about a GRANT that
			// opens the requests list makes the reader find the thing the
			// notification was already holding.
			Link:       "/jit?tab=grants",
...
func (s *JITService) ActivateDueBreakglass(ctx context.Context) (int, int) {
	...
	for _, a := range out {
		...
		s.tell(NotifyInput{
			...
			// The requester's own page has no break-glass tab; the elevation
			// they were just given is a grant, and that is where it is listed.
			Link:       "/jit?tab=grants",
			EntityType: "access_grant",
			EntityID:   a.grant.ID,
