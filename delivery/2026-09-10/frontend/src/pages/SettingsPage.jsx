...
import { UserRound, ShieldCheck, Bell, Laptop, Palette, RotateCcw, Save, Lock, Building2 } from 'lucide-react'
...
import { NetworkAllowlistPanel } from '../components/settings/NetworkAllowlistPanel'
...
export default function SettingsPage() {
  ...
  const SECTIONS = useMemo(
    () => [
      ...
      // Root only, and gated again on the server by middleware.RequireRoot.
      // Hiding it from an admin is a courtesy so nobody is offered a control
      // they cannot use; the 403 is what actually enforces it.
      ...(isRoot ? [{ key: 'organization', label: 'Organization', icon: Building2 }] : []),
    ],
    [isRoot]
  )
    ...
    <Stack gap="lg">
      ...
      <div className="grid gap-6 lg:grid-cols-[13.5rem_minmax(0,1fr)] lg:gap-10">
        ...
        <div className="min-w-0 max-w-4xl">
          ...
          {active === 'organization' && isRoot && (
            <Group
              title="Organization"
              description="Controls that apply to everyone using this deployment, not just your own account."
            >
