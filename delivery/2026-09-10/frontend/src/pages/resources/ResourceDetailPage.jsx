...
import {
  ...
  OpenInWebButton,
  LiveWebSessionBar,
} from '../../components/resources/ResourceAccess'
...
const TABS = [
  ...
  // Restored. An admin looking at a resource is often the person who most
  // needs to open it — and with the brokered web path now available this tab
  // is the only place the three launch routes are shown side by side.
...
export default function ResourceDetailPage() {
  ...
  // Hiding the links is presentation; this is the actual gate. An operator who
  // types /resources/<id>, follows a bookmark, or is sent one by a colleague
  // would otherwise land on a configuration page where every control is
  // disabled and the credential tab reads as an error.
  //
  // It is a redirect rather than a "denied" plate because nothing was denied:
  // the resource is theirs to use, just not to configure, and the list is
  // where using it happens. The server is the real boundary either way —
  // every admin action on this page is behind RequireAdmin regardless of what
  // the console renders.
  if (!isAdmin) {
 ...
 the agent hand-off, the action the label promises. */}
                  ...
                  {/* Renders itself only when the server says this resource can
                      actually be brokered right now, so it is absent rather
                      than present-and-failing. */}
                  <OpenInWebButton resourceId={id} />
                  {isAdmin && (
            ...
            {/* A brokered session is worked in ANOTHER tab, so on returning
                here the operator needs re-entry and a kill switch, not the
                launcher they already used. Renders nothing when none is open. */}
            <LiveWebSessionBar resourceId={id} />
