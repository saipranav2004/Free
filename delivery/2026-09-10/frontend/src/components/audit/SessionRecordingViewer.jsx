...
// ---------------------------------------------------------------------------
// recording.format values. Compared lower-cased because the column has a
// default rather than a constraint, and an unexpected value must fall back to
// the terminal renderer rather than showing a blank stage.
const FORMAT_VISUAL = 'rrweb'
// A desktop application session, captured as H.264/MP4 by the agent's screen
// recorder. Unlike the other two this is not a stream of events the console
// replays: the browser decodes it. See ./VideoStage.jsx.
...
//
// WHAT IT IS, PRECISELY, BECAUSE THE HONESTY MATTERS. There are TWO recording
// formats and this viewer renders both, selected by recording.format — never
// by sniffing the bytes, because an rrweb stream parsed as an asciicast
// silently treats its first event as a header and "works" while being wrong.
//
//   "asciicast"  a terminal transcript ([seconds, stream, data] triples from
//                the gateway or the native agent). Replayed by the ANSI
//                emulator in this file.
//
//   "rrweb"      a VISUAL replay of a brokered web session — DOM mutations,
//                mouse, scroll and input captured in the operator's browser,
//                replayed by ./RrwebStage.jsx. A real screen replay, not a
//                re-render of a screenshot.
//
// There is still no video codec anywhere. A recording is one of the two
// streams above plus a separately stored, sequence-numbered command log. So this player is a terminal replay
...
// rrweb replays come back as the SAME JSONL endpoint but with no header line:
// every line is an event. Parsing them through fetchRecordingCast would eat
// the first event as a header, which is exactly the silent corruption the
// format flag exists to prevent.
async function fetchRecordingReplay(id, signal) {
  if (typeof adminApi.getRecordingReplay === 'function') {
    return adminApi.getRecordingReplay(id, signal)
  }
  const { data } = await requireHttp().get(`/api/v1/pam/admin/recordings/${id}/cast`, {
    signal,
    responseType: 'text',
...
function Viewer({ recording, autoplay, onClose }) {
  ...
  // Visual replay. rrweb owns its own clock — it knows the real duration only
  // after it has loaded the events — so the transport reads progress FROM the
  // stage instead of driving it, which is the one place this player's clock
  // inverts. seekToken forces a seek even when the target time is unchanged
  // (restarting to 0 from 0 must still rewind).
  const isVisual = String(recording?.format || '').toLowerCase() === FORMAT_VISUAL
  const isVideo = String(recording?.format || '').toLowerCase() === FORMAT_VIDEO
  // Both non-terminal formats invert this player's clock: the stage knows the
  // real duration and the real position, and the transport reads them FROM it
  // rather than driving them. rrweb learns its duration only after loading its
  // events; a video learns it from the decoder's metadata. Only the terminal
  // replay has timestamps this component can drive a clock from.
  const stageOwnsClock = isVisual || isVideo
  ...
  // Deliberately NOT run for a video. That artifact is an MP4 served from its
  // own route, and pulling it through the JSONL parser here would read a
  // binary file as text and hand the stage an empty event list.
  const castQuery = useQuery({
    queryKey: ['admin', 'recording', recording.id, 'cast', isVisual ? 'rrweb' : 'asciicast'],
    queryFn: ({ signal }) =>
      isVisual ? fetchRecordingReplay(recording.id, signal) : fetchRecordingCast(recording.id, signal),
    enabled: canReplay && !isVideo,
    retry: false,
  })
  ...
  // A visual replay is a bare array of rrweb events; an asciicast is
  // { header, events }. Both land in castQuery, so unwrap by format rather
  // than by shape — a shape check would quietly pick the wrong branch on an
  // empty recording.
  const events = useMemo(() => {
    if (!castQuery.data) return []
    return isVisual ? castQuery.data : castQuery.data.events || []
  }, [castQuery.data, isVisual])
  const header = (!isVisual && castQuery.data?.header) || {}

  // Event timestamps drive the scrubber ticks and idle-skipping for a
  // terminal replay. rrweb events carry absolute epoch milliseconds, not
  // seconds-from-start, so they are meaningless to this clock and are left
  // empty — the stage reports its own progress instead.
  const times = useMemo(() => {
    if (stageOwnsClock) return []
    return events.map((e) => Number(e?.[0]) || 0)
  }, [events, stageOwnsClock])
  const castDuration = times.length ? times[times.length - 1] : 0
  const recordedDuration = Number(recording?.duration_seconds) || 0
  // When the stage owns the clock, its own duration is the ONLY one the
  // scrubber can honour, so it replaces the record's rather than being maxed
  // with it. The session's duration is a fact about the SESSION; the stage's
  // is a fact about the ARTIFACT, and the two legitimately differ: the agent
  // caps a single screen capture at four hours, so a longer desktop session
  // stores a shorter video. Taking the larger of the two drew a scrubber whose
  // tail could not be reached, and a video that ended with the handle still
  // two thirds of the way along. The record's duration is still the fallback
  // until the stage has loaded enough to report its own, so the transport is
  // never a zero-width bar.
  const duration = stageOwnsClock
    ? visualDuration || recordedDuration
    : Math.max(castDuration, recordedDuration)
  ...
  // Progress reported BY the visual stage. Written straight through to the
  // same t/tRef the terminal clock uses, so every consumer downstream — the
  // scrubber, the command highlight, the timestamp — is unaware of which
  // format is playing.
  const handleVisualProgress = useCallback((seconds) => {
  ...
  useEffect(() => {
    // rrweb and the video element each run their own player and report
    // progress back through onProgress, so a second clock here would fight
    // them: two sources advancing t would make the scrubber jitter and the
    // log highlight land on the wrong command.
    if (stageOwnsClock) return undefined
  ...
  }, [playing, speed, skipIdle, duration, times, stageOwnsClock])
  ...
  const seek = useCallback(
    (to) => {
      ...
      // rrweb only re-seeks when told to. Bumping the token makes an explicit
      // seek land even when the target equals the current time — restarting
      // to 0 while already at 0 must still rewind.
      setSeekToken((n) => n + 1)
    },
    [duration]
  )
          ...
          >
            <div className="relative min-h-0 flex-1 bg-[#080b10]">
              {!canReplay ? (
                ...
                // A desktop application session. VideoStage owns its own
                // loading and failure states because it fetches the artifact
                // itself, so it is placed AHEAD of the cast query's states,
                ...
                <VideoStage
                  ...
                  seekToken={seekToken}
                  seekTo={t}
                  onProgress={handleVisualProgress}
                  onDuration={setVisualDuration}
            ...
            <div className="flex-none border-t border-surface-800 bg-surface-900 px-4 py-3">
              ...
              <div className="mt-2.5 flex flex-wrap items-center gap-x-3 gap-y-2">
                ...
                {/* An event count is meaningful for a stream this component
                    replays itself. A video has no events, so the counter read
                    "event 0 of 0" beside a perfectly good recording. */}
                {stageOwnsClock ? null : (
                ...
                <div className="ml-auto flex flex-wrap items-center gap-2">
                  {/* Idle skipping works by stepping this component's own
                      clock over gaps between event timestamps. A stage that
                      owns its clock has no such gaps to step over, so the
