...
// away in the footer.
export function ResourceDrawer({ resource, open, onClose, isAdmin = false }) {
 ...
 panel doesn't fit here, so it sends you to the page that does. */}
          <OpenInDesktopButton
            ...
            size="sm"
            onNeedsPairing={() => {
              // ?pair=1 is what makes this handoff work. Navigating without
              // it landed the reader on a detail page in its default state,
              // with the pairing panel closed and nothing explaining why they
              // had been moved, so the only trace of the problem was a toast
              // that had already faded. The hook now says the reason out
              // loud; this makes the destination match it.
              onClose?.()
              navigate(`/resources/${resource.id}?pair=1`)
            }}
          />
          {/* Admin only. The full page is a configuration surface —
              credentials, policies, audit, delete — and every part of it is
              already admin-gated, so an operator lands on a page whose only
              working control is the one they just used here. */}
          {isAdmin && (
            <Link
