...
func cmdReportEnd(args []string) error {
	...
	// The tool's own exit status decides how the session is recorded, so a
	// psql that died on an error is not filed as a clean finish.
	status := "COMPLETED"
	if *exitCode != 0 {
		status = "FAILED"
	}
...
func cmdLaunch(args []string, configDir string, configDirErr error) error {
	...
	preparedCmd, chosen, err := launcher.SelectAndBuildCommand(resolved, candidates, sessionTempDir, log)
	if preparedCmd != nil {
	...
	// Shared by both "browser" (e.g. MinIO's console) and "gui" (e.g.
	// pgAdmin4) candidates — see CopyPasswordToClipboard's doc comment for
	// why both need this: neither has a headless way to accept the
	// password, so the closest thing to "already logged in" is landing on
	// the login prompt with the password one paste away.
	if preparedCmd.CopyPasswordToClipboard {
		if err := launcher.CopyToClipboard(resolved.Password); err != nil {
			// Best-effort — see CopyToClipboard's doc comment. The
			// operator can still get the password from PAM's own UI, so
			// this is a warning, not a reason to abort the launch.
			log.Warn("launch.clipboard_copy.fail", "resource_type", resolved.ResourceType, "error", err.Error())
			fmt.Fprintf(os.Stderr, "warning: could not copy password to clipboard: %v\n", err)
		} else {
	...
	case waited:
		...
		if endErr := client.EndLaunch(resolved.SessionID, identity.AgentDeviceID, "COMPLETED",
			identity.PrivateKey(), enforcement, nil); endErr != nil {
			log.Warn("launch.session_end_report.fail", "session_id", resolved.SessionID, "error", endErr.Error())
			fmt.Fprintf(os.Stderr, "warning: failed to report session end to PAM: %v\n", endErr)
		}
