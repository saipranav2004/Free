...
type NetworkConfig struct {
	...
	// SnippetToken authorises the console container to fetch the network
	// allowlist as nginx directives (GET /api/v1/pam/network/allowlist.conf).
	// That fetch happens from an entrypoint script with no operator and no
	// JWT, so it needs a credential of its own.
	//
	// Blank disables the endpoint entirely rather than leaving it open: the
	// response lists every network permitted to reach the console, which is
	// exactly the map an attacker would want, so "not configured" has to
	// mean 404 and not "served to anyone who asks".
	SnippetToken string `mapstructure:"snippet_token"`
}
...
// TrustedProxyState is what this deployment has DECIDED about the hop in
// front of the API, which is not the same question as what the setting
// currently resolves to.
//
// Both "" and "none" produce the same runtime behaviour (trust nothing, use
// the socket peer), and that is exactly why they have to be told apart
// elsewhere. "none" is an operator asserting that this process is reached
// directly, so the address it sees IS the client's. "" is nobody having
// decided anything, which behind a load balancer means every request appears
// to come from the balancer. The allowlist cannot be enabled safely in that
// second state and can be in the first, so the distinction is the whole
// point of this type.
type TrustedProxyState string

const (
	// TrustedProxyUndecided is the default: nothing configured. Safe on a
	// laptop, silently wrong behind any proxy.
	TrustedProxyUndecided TrustedProxyState = "undecided"

	// TrustedProxyDirect is the explicit "none": the operator has asserted
	// that nothing sits in front of this process.
	TrustedProxyDirect TrustedProxyState = "direct"

	// TrustedProxyConfigured means real ranges were given, so
	// X-Forwarded-For is resolved and c.ClientIP() is the operator.
	TrustedProxyConfigured TrustedProxyState = "configured"
)

// TrustedProxyState reports which of the three the configuration expresses.
func (n NetworkConfig) TrustedProxyState() TrustedProxyState {
	raw := strings.TrimSpace(n.TrustedProxies)
	if raw == "" {
		return TrustedProxyUndecided
	}
	if strings.EqualFold(raw, noProxySentinel) {
		return TrustedProxyDirect
	}
	if len(splitList(raw)) == 0 {
		// Punctuation only, e.g. a stray comma. Nothing was really decided.
		return TrustedProxyUndecided
	}
...
func Load() (*Config, error) {
	...
	v.SetDefault("network.snippet_token", "")
