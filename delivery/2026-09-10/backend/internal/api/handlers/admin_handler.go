...
// GetRecordingVideo handles GET /api/v1/pam/admin/recordings/:id/video
//
// A DESKTOP application session, recorded as video by the agent's screen
// recorder (see pam-agent's internal/recorder/screen.go): VP9/WebM where the
// operator's ffmpeg can encode it, H.264/MP4 otherwise. Served on its own
// route rather than through GetRecordingCast for two reasons that are not
// stylistic:
//
//   - the bytes are NOT gzip'd, unlike every other artifact, so the shared
//     path's gzip.NewReader would fail on them immediately; and
//   - a <video> element seeks with byte-range requests, which a plain copy to
//     the response writer cannot answer.
//
// http.ServeContent is what supplies the second: it handles Range, If-Range,
// 206 Partial Content and the Accept-Ranges header, so scrubbing through a
// four-hour session does not download four hours first.
//
// 404s for any recording that is not a video, so a caller cannot use this
// route to fetch a terminal transcript with the wrong content type.
func (h *AdminHandler) GetRecordingVideo(c *gin.Context) {
	if h.storage == nil {
		response.Error(c, http.StatusServiceUnavailable, "Recording storage is not configured")
		return
	}
	...
	if rec.Format != services.RecordingFormatVideo {
		response.Error(c, http.StatusNotFound, "This recording is not a video")
		return
	}
	if rec.StorageKey == "" {
		response.Error(c, http.StatusNotFound, "This recording has no stored artifact yet")
	...
	raw, err := h.storage.Load(c.Request.Context(), rec.StorageKey)
	if err != nil {
		h.logger.Error("admin.recordings.video.load.fail", zap.String("recording_id", rec.ID), zap.Error(err))
		response.Error(c, http.StatusInternalServerError, "Failed to load recording artifact")
		return
	}

	// Derived from the stored key, which is the record of which container the
	// agent's encoder produced. A WebM served as video/mp4 is refused by the
	// browser, so this is not cosmetic.
	mediaType := services.RecordingMediaType(rec.StorageKey)
	ext := path.Ext(rec.StorageKey)
	if ext == "" {
		ext = ".mp4"
	}
	c.Header("Content-Type", mediaType)
	c.Header("Content-Disposition", `inline; filename="`+rec.ID+ext+`"`)
	// A session recording is evidence about one moment and never changes, so
	// it is safe to cache hard; private because it is privileged-access
	// footage and must not sit in a shared cache.
	...
	http.ServeContent(c.Writer, c.Request, rec.ID+ext, modTime, bytes.NewReader(raw))
}
