...
func SelectAndBuildRecordable(resolved *apiclient.ResolvedLaunch, candidates []Candidate, tempDir string, log *slog.Logger, canRecord func(mode string) bool) (*PreparedCommand, *Candidate, error) {
	...
	for i := range ordered {
		...
		// An administrator can pin where this tool lives on the operators'
		// machines, per resource, which is the only workable answer for a
		// tool that ships as a ZIP and is extracted wherever somebody chose.
		// See discovery.Spec.ExplicitPath and toolPathFor.
		spec := cand.Discovery
		spec.ExplicitPath = toolPathFor(resolved.ExtraConfig, cand.ID)
		result := discovery.Locate(spec)
		if !result.Found() {
			tried = append(tried, MissingTool{ID: cand.ID, Kind: cand.Kind, Command: cand.Command, InstallHint: cand.InstallHint})
			// A PINNED path that does not exist is a different event from a
			// tool that was never installed, and it must not be whispered at
			// Debug alongside it. An administrator set that path on the
			// resource; if it is stale, the launch quietly moves on to the
			// next candidate and the session still opens, so nothing else in
			// the system will ever tell them the pin is dead. This log line
			// is the only signal, which is why it is a warning.
			if spec.ExplicitPath != "" {
				log.Warn("launch.candidate.pinned_path_missing",
					"resource_type", resolved.ResourceType, "candidate", cand.ID,
					"configured_path", spec.ExplicitPath,
					"effect", "this tool was skipped; the launch continues with the next candidate if there is one")
				continue
			}
...
// toolPathFor reads an administrator's explicit path for one candidate out of
// the resource's extra_config.
//
// Two spellings are accepted, because two different questions get asked:
//
//	{"tool_path": "D:\\tools\\sqldeveloper\\sqldeveloper.exe"}
//	  the path for whichever tool this resource opens with, which is what an
//	  admin means when the resource only really has one usable client.
//
//	{"tool_paths": {"sqldeveloper": "D:\\tools\\sqldeveloper\\sqldeveloper.exe"}}
//	  per candidate, for a resource where more than one client is in play and
//	  only one of them lives somewhere unusual.
//
// The per-candidate form wins when both are present, since it is the more
// specific statement. Anything that is not a string is ignored rather than
// guessed at.
func toolPathFor(extraConfig map[string]interface{}, candidateID string) string {
	if extraConfig == nil {
		return ""
	}
	if byTool, ok := extraConfig["tool_paths"].(map[string]interface{}); ok {
		if p, ok := byTool[candidateID].(string); ok && strings.TrimSpace(p) != "" {
			return strings.TrimSpace(p)
		}
	}
	if p, ok := extraConfig["tool_path"].(string); ok {
		return strings.TrimSpace(p)
	}
	return ""
}
