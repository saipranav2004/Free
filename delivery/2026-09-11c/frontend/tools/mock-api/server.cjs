...
on('GET', '/api/v1/auth/me', (ctx) => {
  ...
  const body = {
    ...
    roles: live.roles || [],
    status: live.status,
    // MOCK_OMIT_MFA_ENABLED reproduces a deployment on the older API, which
    // never sent this field. That is what made the enrolment banner permanent:
    // the console read the absent field as "not enrolled" and asked people who
    // already had a second factor to set one up, on every page.
  ...
  // MOCK_OMIT_MFA_ENABLED reproduces a deployment on the older API, which
  // computed enrolment on every request and then never put it in the response.
  // That is what made the enrolment banner permanent: the console read the
  // absent field as "not enrolled" and asked people who already had a second
  // factor to set one up, on every page. Deleted rather than skipped above,
  // because the user row is spread into the body and carries the key too.
  if (process.env.MOCK_OMIT_MFA_ENABLED) delete body.mfa_enabled
  return ok(ctx.res, body)
})
...
on('GET', '/api/v1/pam/resources/:id/connect-info', (ctx) => {
  ...
  return ok(ctx.res, {
    ...
    // The real handler reports this and the panel disables its launch button
    // without it, so the mock has to as well or the launch path is not
    // drivable here at all.
    has_credential: hasCredential(r),
    // available_methods is what decides which Connect buttons exist at all
    // (ResourceAccess.jsx reads it and renders nothing for a method it does
    // not name). Without it here the BROKERED web path was unreachable in the
    // mock, which is why it had never been driven in a browser.
    available_methods: availableMethods(r),
  })
})

// Mirrors resource_handler.go ConnectInfo. Kept as functions so the resource
// list and the web-session routes below agree with what the panel was told.
function hasCredential(r) {
  // res-03 and res-09 carry no credential fixture but are the two resources
  // the launch paths are exercised on: res-03 for the native agent, res-09
  // (Langfuse) for the brokered web session.
  return Boolean(r.vault_entry_id) || r.id === 'res-03' || r.id === 'res-09'
}

// gateway.SupportedResourceTypes: the types PAM can speak a live protocol for.
const WEB_TERMINAL_TYPES = new Set(['redis', 'postgresql', 'mongodb', 'ssh'])

function availableMethods(r) {
  const methods = []
  const cred = hasCredential(r)
  if (cred && WEB_TERMINAL_TYPES.has(r.resource_type)) methods.push('web_terminal')
  if (cred) methods.push('native_agent')
  // web_proxy is the brokered path: PAM signs in server-side and proxies the
  // app, so it needs a credential and a console URL. web_app is the raw link.
  if (cred && r.console_url) methods.push('web_proxy')
  if (r.console_url) methods.push('web_app')
  return methods
}
// ── Brokered web sessions (internal/webproxy) ─────────────────────────────
//
// The third connect method, and the one the mock had NO routes for at all,
// which is why the "Open in Web" button could never be clicked here: it is
// gated on available_methods carrying web_proxy, and connect-info never said
// so. Both halves are now modelled.
//
// auth_strategy is the field worth reading: it is how the console reports
// WHICH server-side login PAM performed, and it mirrors the backend's
// Registry.Get inference exactly.
function authStrategyFor(r) {
  switch (String(r.resource_type || '').toLowerCase()) {
    case 'minio':
      return 'minio_console'
    case 'langfuse':
      // Langfuse authenticates with NextAuth's credentials provider, so PAM
      // signs in server-side and the operator lands inside the app rather
      // than on its sign-in form.
      return 'nextauth_credentials'
    default:
      return 'none'
  }
}
db.webSessions = db.webSessions || []
on('POST', '/api/v1/pam/resources/:id/web-session', (ctx) => {
  const r = db.resources.find((x) => x.id === ctx.params.id)
  if (!r) return fail(ctx.res, 404, 'NOT_FOUND', 'That resource does not exist.')
  if (!availableMethods(r).includes('web_proxy')) {
    return send(ctx.res, 403, {
      success: false,
      code: 'connect_method_not_allowed',
      error: 'This resource does not permit the brokered web connect method',
    })
  }
  const id = uid('wps')
  const sub = `${r.name.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}-${id.slice(-6)}`
  const ws = {
    id,
    resource_id: r.id,
    ...
    status: 'ACTIVE',
    auth_strategy: authStrategyFor(r),
    app_url: `http://${sub}.pam.local:8787/`,
    started_at: new Date().toISOString(),
  }
  db.webSessions.unshift(ws)
  auditRow(ctx.user, 'SESSION', 'webproxy.session.started', 'SUCCESS', `resource:${r.name}`)
  return ok(ctx.res, {
    web_proxy_session_id: ws.id,
    session_id: uid('sess'),
    // Single use and short lived, exactly as the real handler describes it.
    launch_url: `${ws.app_url}__pam/handoff?token=${uid('hof')}`,
    app_url: ws.app_url,
    auth_strategy: ws.auth_strategy,
    expires_at: F.iso(60),
    handoff_expires_at: F.iso(0.5),
  })
})
on('GET', '/api/v1/pam/web-sessions/mine', (ctx) =>
  ok(ctx.res, { sessions: db.webSessions.filter((w) => w.user_id === ctx.user.user_id && w.status === 'ACTIVE') }))
on('POST', '/api/v1/pam/web-sessions/:id/end', (ctx) => {
  // Ownership-checked, and someone else's session is reported as missing
  // rather than forbidden, which is what the real handler does.
  const ws = db.webSessions.find((w) => w.id === ctx.params.id && w.user_id === ctx.user.user_id)
  if (!ws) return fail(ctx.res, 404, 'NOT_FOUND', 'That session does not exist.')
  ws.status = 'ENDED'
  ws.ended_at = new Date().toISOString()
  return ok(ctx.res, { web_proxy_session_id: ws.id, status: ws.status })
})
...
// A desktop-application recording: the console asks for the video bytes on
// their own route, so the mock serves the real capture produced by the agent's
// screen recorder when one is present (MOCK_VIDEO_FILE), which is what makes
// the video player testable without running the whole backend. MOCK_VIDEO_TYPE
// selects the container, since the agent sends WebM or MP4 depending on what
// the operator's ffmpeg could encode.
on('GET', '/api/v1/pam/admin/recordings/:id/video', (ctx) => {
  const file = process.env.MOCK_VIDEO_FILE
  if (!file || !fs.existsSync(file)) return fail(ctx.res, 404, 'NOT_FOUND', 'This recording is not a video.')
  const body = fs.readFileSync(file)
  ctx.res.writeHead(200, {
    'Content-Type': process.env.MOCK_VIDEO_TYPE || 'video/webm',
...
// ── Network allowlist (root only) ────────────────────────────────────────
// Modelled with the proxy state the real handler reports, because the panel's
// most important behaviour is what it does when the API cannot vouch for the
// address it is showing. MOCK_PROXY_STATE selects it: undecided | direct |
// configured. MOCK_YOUR_IP selects the address the API claims to see.
const allowlist = { enabled: false, entries: [] }
const yourIp = () => process.env.MOCK_YOUR_IP || '203.0.113.9'
const proxyState = () => process.env.MOCK_PROXY_STATE || 'configured'
const isInfra = (ip) =>
  /^10\./.test(ip) || /^192\.168\./.test(ip) || /^169\.254\./.test(ip) ||
  /^172\.(1[6-9]|2\d|3[01])\./.test(ip) || /^100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\./.test(ip)

on('GET', '/api/v1/pam/admin/network/allowlist', (ctx) =>
  ok(ctx.res, {
    enabled: allowlist.enabled,
    entries: allowlist.entries,
    your_ip: yourIp(),
    your_ip_is_allowed: allowlist.entries.some((e) => e.cidr === `${yourIp()}/32`),
    your_ip_is_infrastructure: isInfra(yourIp()),
    proxy_state: proxyState(),
  }), 'admin')
on('POST', '/api/v1/pam/admin/network/allowlist', (ctx) => {
  const raw = String(ctx.body?.cidr || '').trim()
  if (!raw) return fail(ctx.res, 400, 'VALIDATION_FAILED', 'Enter an IP address or CIDR range')
  const cidr = raw.includes('/') ? raw : `${raw}/32`
  const entry = { id: uid('nal'), cidr, label: ctx.body?.label || '', added_by_username: ctx.user.username, created_at: new Date().toISOString() }
  allowlist.entries.push(entry)
  return ok(ctx.res, entry, 201)
}, 'admin')
on('DELETE', '/api/v1/pam/admin/network/allowlist/:id', (ctx) => {
  allowlist.entries = allowlist.entries.filter((e) => e.id !== ctx.params.id)
  return ok(ctx.res, null)
}, 'admin')
on('PUT', '/api/v1/pam/admin/network/allowlist/enabled', (ctx) => {
  const want = Boolean(ctx.body?.enabled)
  if (want && proxyState() === 'undecided' && isInfra(yourIp())) {
    return send(ctx.res, 409, {
      success: false,
      error: `the API cannot tell whose address it is seeing: it sees you at ${yourIp()}, which is a private range, and no trusted proxy set is configured`,
    })
  }
  allowlist.enabled = want
  return ok(ctx.res, { enabled: allowlist.enabled })
}, 'admin')
...
on('GET', '/api/v1/pam/agent/devices', (ctx) => {
  ...
  // Capabilities, as a real agent reports them at pair time. MOCK_DEVICE_CAPS
  // selects the shape under test:
  //   full    every type openable        (the pre-flight should let a launch through)
  //   nomongo mongodb missing, rest fine (the pre-flight should stop it and say so)
  //   none    paired but reported nothing (older agent: fall through, never guess)
  const mode = process.env.MOCK_DEVICE_CAPS || 'full'
  const caps = {
    full: [
      { resource_type: 'postgresql', available: true, tool: 'psql', kind: 'cli' },
      { resource_type: 'mongodb', available: true, tool: 'mongosh', kind: 'cli' },
    ],
    nomongo: [
      { resource_type: 'postgresql', available: true, tool: 'psql', kind: 'cli' },
      {
        ...
        available: false,
        install_hint:
          'Install mongosh from mongodb.com/try/download/shell, or MongoDB Compass for a desktop client.',
      },
    ],
  }[mode]
  const withCaps = rows.map((d) => (caps ? { ...d, capabilities: caps } : d))
  return ok(ctx.res, { devices: withCaps, count: withCaps.length })
})
// The platforms an operator can be handed a one-click installer for.
//
// This was MISSING from the mock, and its absence is why the enrolment panel
// had never been exercised in a test: with no targets the platform picker is
// empty, the installer can never be generated, and the panel's "watch for the
// agent pairing" logic never arms. Every check of that flow silently tested
// nothing.
on('GET', '/api/v1/pam/agent/install/targets', (ctx) =>
  ok(ctx.res, {
    enabled: true,
    targets: [
      { os: 'windows', arch: 'amd64', label: 'Windows (x64)', size_bytes: 8568320, sha256: 'bf513ab52cd39b9fbe92bb888ee4efd986479d947687d178e5fdf157650f0763', version: '2026.09.11' },
      { os: 'darwin', arch: 'arm64', label: 'macOS (Apple silicon)', size_bytes: 8037634, sha256: '762920cf872950fd800cfcfed3b53c43f4631c38d778d845fa26f5cc9f3eb767', version: '2026.09.11' },
      { os: 'linux', arch: 'amd64', label: 'Linux (x64)', size_bytes: 8290488, sha256: 'ab01c2314479bd94161f4876bb87b269308137c93c5a4de30e204a3148b983a6', version: '2026.09.11' },
    ],
  })
)

// The one-click installer for a chosen platform. Also absent before, for the
// same reason and with the same effect: the panel could not arm.
on('POST', '/api/v1/pam/agent/install/bootstrap', (ctx) => {
  const os = (ctx.body && ctx.body.os) || 'windows'
  const arch = (ctx.body && ctx.body.arch) || 'amd64'
  const ttl = 900
  return ok(ctx.res, {
    target: { os, arch },
    filename: os === 'windows' ? 'pam-agent-setup.ps1' : 'pam-agent-setup.sh',
    command: os === 'windows'
      ? 'powershell -ExecutionPolicy Bypass -File .\\pam-agent-setup.ps1'
      : 'sh ./pam-agent-setup.sh',
    script_url: '/api/v1/pam/agent/install/script/' + uid('tok'),
    script: '# one-click setup, embeds a single-use pairing code\n',
    expires_at: new Date(Date.now() + ttl * 1000).toISOString(),
    ttl_seconds: ttl,
  })
})

// The agent's own pairing, as the SERVER performs it. The browser never calls
// this; it exists so the enrolment panel's "waiting for the agent" behaviour
// can be driven from a test the way a real installer would drive it.
//
// It mirrors the real CompletePairing exactly, INCLUDING the part that matters
// here: a machine that is already paired has its row updated rather than a
// second row added. A mock that always appended would have hidden the fact
// that the panel only ever noticed a FIRST pairing.
on('POST', '/api/v1/pam/agent/__test/pair', (ctx) => {
  const name = (ctx.body && ctx.body.device_name) || 'TEST-MACHINE (windows)'
  const now = new Date().toISOString()
  const existing = db.devices.find(
    (d) => d.user_id === ctx.user.user_id && d.device_name === name && d.status !== 'REVOKED'
  )
  if (existing) {
    existing.last_seen_at = now
    existing.status = 'ACTIVE'
    return ok(ctx.res, { device: existing, new_device: false })
  }
  const row = {
    id: uid('dev'), user_id: ctx.user.user_id, device_name: name,
    status: 'ACTIVE', last_seen_at: now, created_at: now,
  }
  db.devices.push(row)
  return ok(ctx.res, { device: row, new_device: true })
})
