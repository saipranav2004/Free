...
import { createLaunch, getLaunchStatus, isNotPairedError, listAgentDevices } from '../../api/agent'
import {
  readAgentReadiness,
  readinessMessage,
  NOT_PAIRED,
...
// points.
// How long to keep asking what became of a launch.
//
...
// without leaving a spinner on screen when nothing is listening at all. The
// launch token's own lifetime is the real ceiling: once it expires unredeemed
// the backend reports "expired", which IS the answer, so polling past that
// point learns nothing new.
const LAUNCH_POLL_MS = 1200
const LAUNCH_POLL_TIMEOUT_MS = 45000

export function useDesktopLaunch(resourceId, { onNeedsPairing, onLaunchState, resourceType, resourceName } = {}) {
  // A ref, not state: the poll loop must not restart on re-render, and
  ...
  // Stop when the component goes away, or an unmounted panel keeps asking the
  // backend about a launch nobody is looking at.
  useEffect(() => stopPolling, [stopPolling])

  // What this account's machines said they can open. Cheap, cached, and read
  // before every launch so the answer is current without another round trip
  // at click time.
    ...
    (launchId) => {
      ...
      const tick = async () => {
        ...
        try {
          ...
          // opened, completed and expired are all final as far as this
          // browser is concerned: from there the session's own lifecycle is
          // the story, and a long-lived poll would outlive the page.
          ...
          // be opening perfectly well and only the reporting channel is
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      // PRE-FLIGHT, before a session row is opened on the server.
      //
      // The machine told PAM what it can open when it paired, so a launch
      // that cannot possibly work is stopped here with something the operator
      // can act on, instead of opening a session, handing off to an agent
      // that has no client installed, and reporting the reason into a log
      // file. Only states the agent actually asserted are acted on: "unknown"
      // falls through and launches, because telling somebody to install
      // software they may already have is worse than letting it try.
      const readiness = readAgentReadiness(devicesQuery.data?.devices, resourceType)
      if (readiness.state === NOT_PAIRED || readiness.state === MISSING_TOOL) {
        const msg = readinessMessage(readiness, resourceName)
        const err = new Error(msg.title)
        err.preflight = { ...readiness, ...msg }
        throw err
      }
      return createLaunch(resourceId)
    },
    onSuccess: (data) => {
    ...
    onError: (err) => {
      ...
      // A pre-flight refusal already knows exactly what is wrong and what to
      // do about it, so it is reported as itself rather than squeezed through
      // the generic API-error path.
      if (err?.preflight) {
        const p = err.preflight
        toast.error(p.title, { description: p.body })
        if (p.state === NOT_PAIRED) onNeedsPairing?.()
        onLaunchState?.({ state: 'failed', failure_reason: p.body, failure_code: p.state })
        return
      }
      if (isNotPairedError(normalized)) {
        // Said out loud. Previously this called onNeedsPairing with no
        // message at all, so an entry point that could not host the pairing
        // panel (the resource drawer) showed nothing whatsoever.
...
export function ResourceAccessPanel({ resource, resourceId, compact = false }) {
  ...
  // What became of the last desktop launch. See LaunchOutcome: a handoff to
  // pam-agent:// is one-way, so without this the panel could not tell a
  // working session from a missing tool from a missing agent.
  ...
  const launchMutation = useDesktopLaunch(resourceId, {
    ...
    // What the pre-flight compares against the paired machine's report.
...
//
// A handoff to pam-agent:// is one-way: the OS takes over and nothing comes
// back. Until the backend started recording launch outcomes there was nothing
// to render here and nothing rendered, so a missing psql, a missing agent and
// a perfectly good session all looked identical.
//
// Deliberately quiet on success. An operator whose tool just opened is looking
// at the tool, not at this page.
function LaunchOutcome({ state }) {
  ...
  // Deliberately does NOT promise that closing the tool ends the session. It
  // does for a terminal or a desktop application, where the agent is still
  // watching. It does not for a resource whose candidate opens the operator's
  // own browser: nothing can observe that tab, which is what PAM's brokered
  // web proxy exists for. The console cannot tell the two apart from here, so
  // it states only what it knows.
  ...
  // plainly is the difference between a diagnosable problem and a button that
  // does nothing.
...
function LaunchProblem({ title, detail, hint }) {
  return (
    <div className="mt-4 flex items-start gap-3 rounded-lg border-l-[3px] border-red-500 bg-red-50/60 px-3 py-3 dark:bg-red-950/15">
      <Download
        className="mt-0.5 h-4 w-4 flex-none text-red-600 dark:text-red-400"
        strokeWidth={1.9}
        aria-hidden="true"
      ...
      <div className="min-w-0">
        ...
        {hint ? <p className="mt-1.5 text-sm leading-relaxed text-ink-500 dark:text-ink-400">{hint}</p> : null}
      </div>
    </div>
  )
}
...
export function OpenInDesktopButton({
  ...
  const launch = useDesktopLaunch(resourceId, { onNeedsPairing, resourceType, resourceName })
  return (
    <Button
      variant="primary"
      size={size}
