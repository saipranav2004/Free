...
import {
  X, Play, Pause, RotateCcw, SkipBack, SkipForward, Maximize2, Minimize2,
  Download, Copy, Check, Terminal, ListTree, ShieldAlert, Gauge,
  Clock, Server, UserRound, HardDrive, Hash, FileText, SearchX, Info,
} from 'lucide-react'
...
async function saveRecordingCast(id, filenameHint) {
  ...
  const response = await requireHttp().get(`/api/v1/pam/admin/recordings/${id}/cast`, { responseType: 'blob' })
...
function Viewer({ recording, autoplay, onClose }) {
  ...
  const markers = useMemo(
    () =>
      duration > 0
        ? commands
            .map((cmd) => ({ cmd, offset: offsetOf(cmd) }))
      ...
      >
        ...
        <header className="flex flex-none flex-wrap items-start gap-x-6 gap-y-3 border-b border-surface-800 bg-surface-850/60 px-5 py-3.5">
          <div className="flex min-w-0 flex-1 items-start gap-3">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge className={RECORDING_STATUS_BADGE[recording.status]}>{recording.status || 'UNKNOWN'}</Badge>
                {riskyCount > 0 && (
          ...
          <div className="flex flex-none items-center gap-2">
            ...
            />
            <IconButton icon={X} variant="ghost" size="sm" onClick={onClose} aria-label="Close" title="Close (Esc)" />
          </div>
        </header>
        ...
        <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
          {/* replay */}
          <section className="flex min-h-0 flex-1 flex-col border-b border-surface-800 lg:border-b-0 lg:border-r">
              ...
              {!canReplay ? (
                <div className="flex h-full items-center justify-center px-6 text-center">
                  <div>
                    ...
                    <p className="mx-auto mt-1.5 max-w-sm text-xs leading-relaxed text-ink-500">
                      The transcript is written when the session ends. Come back once the status is
                      COMPLETED and the replay will be available here.
                    </p>
                  </div>
                </div>
              ) : castQuery.isLoading ? (
                <div className="space-y-2 p-4">
                  {[0, 1, 2, 3, 4, 5, 6, 7].map((i) => (
                    <div key={i} className="skeleton h-3 rounded" style={{ width: `${40 + ((i * 13) % 55)}%` }} />
                  ))}
                </div>
              ) : castQuery.isError ? (
                <div className="flex h-full items-center justify-center px-6">
                  <div className="max-w-sm text-center">
                    ...
                    <p className="mt-1.5 text-xs leading-relaxed text-ink-500">{apiErrorMessage(castQuery.error)}</p>
                    <Button size="sm" variant="secondary" className="mt-4" onClick={() => castQuery.refetch()}>
                      Retry
                    </Button>
                  </div>
                </div>
                ...
                <div className="flex h-full items-center justify-center px-6">
                  <p className="max-w-sm text-center text-xs leading-relaxed text-ink-500">
                    This recording contains no transcript data. The session may have produced no
          ...
          <aside className="flex min-h-0 w-full flex-col bg-surface-900 lg:w-[27rem] xl:w-[31rem]">
            <div className="flex flex-none items-center gap-1 border-b border-surface-800 px-3">
              {[
                { key: 'log', label: 'Command log', icon: ListTree, count: pagination?.total ?? commands.length },
            ...
            {tab === 'log' ? (
                            ...
                            >
                              <span
                                ...
                                className={clsx(
                                  ...
                                  risky ? 'bg-red-400/80' : active ? 'bg-blue-500' : reached ? 'bg-emerald-500/45' : 'bg-transparent'
                                )}
                              />
                              ...
                              <span className="min-w-0 flex-1">
                                ...
                                <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-2xs text-ink-500">
                                  ...
                                  {cmd?.output_summary && <span className="truncate">{cmd.output_summary}</span>}
                                </span>
                              </span>
              ...
              <div className="min-h-0 flex-1 overflow-y-auto">
                <div className="divide-y divide-surface-800">
                  <MetaField icon={Server} label="Resource" value={recording.resource_name || 'Not reported'} />
                  <MetaField icon={Hash} label="Resource ID" value={recording.resource_id || 'Not reported'} mono />
                  <MetaField icon={UserRound} label="User" value={recording.username || 'Not reported'} />
                  <MetaField icon={Hash} label="Session ID" value={recording.session_id || 'Not reported'} mono />
                  ...
                  />
                  {recording.ended_at && (
                    <MetaField icon={Clock} label="Ended (local)" value={formatDateTime(recording.ended_at)} />
                  )}
                  <MetaField
                    icon={Gauge}
                    label="Duration"
                    ...
                    <li>
                      {riskyCount > 0 ? (
                        <span className="text-red-700 dark:text-red-300">
                          {riskyCount} command{riskyCount === 1 ? '' : 's'} on this page ended in an
                          error or denial.
                        </span>
                      ) : (
                ...
                <div className="border-t border-surface-800 px-4 py-4">
                  ...
                  <div className="mt-2.5 flex flex-wrap gap-2">
                    ...
                    <Button size="sm" variant="secondary" icon={FileText} onClick={downloadLog}>
                      Command log
                    </Button>
                  </div>
                  <p className="mt-2.5 flex items-start gap-2 text-2xs leading-relaxed text-ink-500">
                    ...
                    The transcript downloads exactly as the gateway stored it. The command log
                    exports the entries currently loaded, which is this page of the log.
                  </p>
                </div>
              </div>
            )}
        ...
        <footer className="flex flex-none flex-wrap items-center gap-x-4 gap-y-1 border-t border-surface-800 bg-surface-850/60 px-5 py-2">
          <span className="text-2xs text-ink-500">
            <kbd className="rounded border border-surface-700 bg-surface-900 px-1 font-mono">Space</kbd> play or pause
          </span>
          <span className="text-2xs text-ink-500">
            <kbd className="rounded border border-surface-700 bg-surface-900 px-1 font-mono">←</kbd>
            <kbd className="ml-1 rounded border border-surface-700 bg-surface-900 px-1 font-mono">→</kbd> seek 10s
          </span>
          <span className="text-2xs text-ink-500">
            <kbd className="rounded border border-surface-700 bg-surface-900 px-1 font-mono">F</kbd> fullscreen
          </span>
          <span className="text-2xs text-ink-500">
            <kbd className="rounded border border-surface-700 bg-surface-900 px-1 font-mono">Esc</kbd> back to list
          </span>
...
export function SessionRecordingViewer({ recording, autoplay = false, onClose }) {
  ...
  return <Viewer key={recording.id || recording.session_id} recording={recording} autoplay={autoplay} onClose={onClose} />
}
