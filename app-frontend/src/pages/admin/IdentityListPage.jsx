...
import { Ban, Copy, Download, KeyRound, Trash2, UserPlus, Users } from 'lucide-react'
import clsx from 'clsx'
import { toast } from 'sonner'
import { listUsers, getUser, setUserStatus, deleteUser } from '../../api/identity'
...
//
// BULK DISABLE AND DELETE FAN OUT, one request per selected account.
//
// /admin/identity still exposes no batch route, and these buttons used to sit
// permanently disabled because of it. That was the wrong call: a batch route
// is the server's convenience, not the operator's requirement, and an admin
// disabling eleven leavers does not care how many HTTP requests it costs. What
// actually matters about fanning out is that it can PARTIALLY succeed, so the
// rule here is that a partial result is reported as one, never rounded to
// "done" or "failed":
//
//   * a bounded number run at a time, so selecting sixty rows does not open
//     sixty sockets at once
//   * every row is awaited, one failure never cancels the rest
//   * the outcome names how many of how many, and the first reasons why
//
// Rows the server would certainly refuse are dropped BEFORE anything is sent,
// with the count said out loud: your own account (disabling or deleting it
// logs you out for good) and protected accounts (the seeded root). Sending
// requests that are guaranteed to 403 just to report them as failures would
// be noise dressed up as diligence.
...
export default function IdentityListPage() {
  ...
  // Accounts this action must not be sent for, and why. Read at confirm time
  // AND again at run time, so a stale selection cannot slip one through.
  const partitionSelection = (rows) => {
    const eligible = []
    const skipped = []
    for (const u of rows) {
      if (u.user_id === viewerId) skipped.push({ u, why: 'your own account' })
      else if (u.is_protected) skipped.push({ u, why: 'protected' })
  ...
  const BULK = {
    disable: {
      ...
      run: (u) => setUserStatus(u.user_id, 'DISABLED'),
      describe: (n) =>
        `${n} ${n === 1 ? 'account is' : 'accounts are'} signed out and cannot sign in again until re-enabled. Sessions already open are not killed by this.`,
    },
    delete: {
      verb: 'deleted',
      title: 'Delete selected accounts?',
      confirmLabel: 'Delete',
      run: (u) => deleteUser(u.user_id),
      describe: (n) =>
        `${n} ${n === 1 ? 'account' : 'accounts'} will be removed. Their audit history remains, the trail is append-only, but any credentials or grants they held stop working immediately. Disabling is usually the better answer.`,
    },
  }

  // Four at a time. Enough that fifty rows do not feel serial, small enough
  // that the API is never hit with the whole selection at once.
  const BULK_CONCURRENCY = 4

  const runBulkAction = async (key) => {
    const spec = BULK[key]
    if (!spec) return
    const { eligible, skipped } = partitionSelection(table.selectedRows)
    setBulkBusy(true)
    ...
    const worker = async () => {
      while (cursor < eligible.length) {
        const u = eligible[cursor++]
        try {
          await spec.run(u)
          ok += 1
        } catch (e) {
          failures.push({ username: u.username, message: normalizeApiError(e).message })
        }
      }
    }
    await Promise.all(
    ...
    setBulkBusy(false)
    setBulkConfirm(null)
    setBulkResult({ ok, failed: failures.length, total: eligible.length, verb: spec.verb })
    table.clearSelection()
    // Covers this list AND every open account record, both live under the
    // same prefix, so a disabled account does not read as active on the
    // detail page a click later.
    queryClient.invalidateQueries({ queryKey: ['admin', 'users'] })

    if (failures.length === 0 && eligible.length > 0) {
      toast.success(`${ok} ${ok === 1 ? 'account' : 'accounts'} ${spec.verb}`)
    } else if (failures.length > 0) {
      toast.error(`${failures.length} of ${eligible.length} could not be ${spec.verb}`, {
    ...
    if (skipped.length > 0) {
      toast.message(
        `${skipped.length} ${skipped.length === 1 ? 'account was' : 'accounts were'} left alone`,
        { description: skipped.map((s2) => `${s2.u.username} (${s2.why})`).join(', ') }
      )
    }
  }
  ...
  const bulkActions = [
    ...
    { key: 'disable', label: 'Disable', icon: Ban, onClick: () => setBulkConfirm('disable') },
    {
      key: 'delete',
      label: 'Delete',
      icon: Trash2,
  ...
  return (
    <Stack gap="lg">
      ...
      {/* The endpoint caps how many accounts one read returns. Saying so is
          the difference between "this install has 1000 users" and "this is the
          first 1000 of them", and only one of those is true. */}
      {usersQuery.data?.truncated && (
        <TruncationNotice
          limit={usersQuery.data.limit}
          noun="accounts"
          hint="Search by username, email or full name to find the rest."
        />
      )}
                ...
                <tr>
                  <Th sticky left="left-0">
                    <SelectAll
                      ...
                      // toggleAllOnPage, not a hand-rolled pair of calls. It
                      // already decides which way to go from allOnPageSelected
                      // AND handles the inverted "all matching" mode, where
                      // the id set holds exclusions rather than inclusions.
                      // The pair this replaced named a method the hook does
                      // not export, so the header checkbox threw on every
                      // click and selected nothing.
      ...
      {/* One dialog for both actions. The count it quotes is the ELIGIBLE
          count, not the selected count, so the number in the question is the
          number of accounts that will actually change. */}
      {bulkConfirm && (
        <ConfirmDialog
          open
          title={BULK[bulkConfirm].title}
          description={(() => {
            const { eligible, skipped } = partitionSelection(table.selectedRows)
            if (eligible.length === 0) {
              return 'Nothing in this selection can be changed. Your own account and protected accounts are always left alone.'
            }
            const main = BULK[bulkConfirm].describe(eligible.length)
            return skipped.length > 0
              ? `${main} ${skipped.length} of the selected accounts ${skipped.length === 1 ? 'is' : 'are'} left alone: ${skipped.map((s2) => `${s2.u.username} (${s2.why})`).join(', ')}.`
              : main
          })()}
          confirmLabel={BULK[bulkConfirm].confirmLabel}
          destructive
          isLoading={bulkBusy}
