...
import { getMfaCompliance } from '../../api/mfaPolicy'
...
export default function IdentityDetailPage() {
  ...
  // THIS CARD WAS FETCHED ONCE AND NEVER AGAIN.
  //
  // No interval, no focus refetch, and the client-wide default is
  // refetchOnWindowFocus:false, so the account record was read at mount and
  // then frozen. Failed sign-ins climbing, a lockout starting or expiring, a
  // status change made by another administrator: none of it reached the
  // screen until the page was navigated away from and back. Sitting on an
  // account during an incident and watching nothing move is exactly the
  // report.
  //
  // staleTime:0 overrides the client-wide 10s, because returning to the tab
  // is the moment this has to be re-read, and refetchOnWindowFocus only
  // refetches a query it already considers stale.
  ...
  // WHERE "SECOND FACTOR" ACTUALLY COMES FROM.
  //
  // The card used to read user.mfa_enabled off the account record. That column
  // is never written true by anything in the backend, so it reported "Not set
  // up" for every account in the install including ones that had just
  // enrolled. The compliance report answers the same question from the DEVICE
  // TABLE (MFAPolicyService.activeMFAUserIDs), which is the fact rather than a
  // copy of it, and it carries whether a policy gates this account as well.
  //
  // Not fatal if it fails: this is one row of one card, and an administrator
  // should not lose the whole account page because a policy report is
  // unavailable. The row says so instead of guessing.
  const complianceQuery = useQuery({
    queryKey: ['admin', 'mfa-compliance'],
  ...
  const mfaRow = (complianceQuery.data?.rows || []).find(
    (r) => r.user_id === id || r.user_id === userQuery.data?.user?.user_id
  )
                  ...
                  <Card>
                    ...
                    <DetailList
                      items={[
                        {
                          label: 'Second factor',
                          value: complianceQuery.isError ? (
                            <span className="text-ink-500">Not reported</span>
                          ) : !mfaRow ? (
                            <span className="text-ink-500">Checking…</span>
                          ) : (
                            <span className="flex flex-wrap items-center gap-x-2 gap-y-1">
                              <StatusDot
                                tone={mfaRow.mfa_enabled ? 'ok' : 'warn'}
                                label={mfaRow.mfa_enabled ? 'Enrolled' : 'Not set up'}
                              />
                              {/* Gated and not enrolled is the finding that
                                  turns a gap into an action: this account is
                                  required to hold a factor and does not. */}
                              {!mfaRow.mfa_enabled && mfaRow.required && (
                                <span className="text-xs text-warn">
                                  required by {(mfaRow.gated_by || []).join(', ') || 'policy'}
                                </span>
                              )}
                            </span>
                          ),
                        ...
                        user.locked_until
                          ? {
                              label: 'Locked until',
                              // THE UNLOCK IS RIGHT HERE, and it was not.
                              // Clearing a lockout has always been possible,
                              // through "set status to Active", which also
                              // resets failed_login_attempts and locked_until
                              // server-side. Nothing said so: an administrator
                              // looking at a locked colleague saw a timestamp
                              // and no way to act on it, and the one control
                              // that would have worked is filed under a status
                              // dropdown that reads like a different subject.
                              value: (
                                <span className="flex flex-wrap items-center gap-2">
                                  <span className="text-warn">{formatDateTime(user.locked_until)}</span>
                                  <Button
                                    size="xs"
                                    variant="subtle"
                                    icon={Unlock}
                                    ...
                                    title="Clears the lockout and the failed sign-in count so this account can sign in again"
                                  >
                                    Unlock now
                                  </Button>
                                </span>
