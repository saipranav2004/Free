...
// Same visual contract for a native <select>, INCLUDING the caret.
//
// Keeping the OS caret was the wrong half of a right instinct. The element
// stays native, so the menu, the keyboard behaviour and the mobile picker are
// all still the platform's; but the closed control is what people see 99% of
// the time, and an OS caret next to our own inputs is the single loudest
// "unfinished" tell in a screenshot. index.css draws the chevron instead (one
// rule, every select in the product, including ones nobody has written yet).
export const selectClass = (hasError) => clsx(inputClass(hasError), 'cursor-pointer pr-9')
