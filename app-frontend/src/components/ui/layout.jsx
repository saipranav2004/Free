...
export function Container({ header, footer, children, className, padded = true, id }) {
  return (
    <section
      id={id}
      className={clsx(
        // shadow-card was `none` for the whole life of this component, so a
        // panel was a bordered rectangle on a page of the same colour. It now
        // rests on the ground with a contact shadow, and it arrives rather
        // than appearing. Both come from the shared tokens, so every panel in
        // the console changed with this one line.
        'surface-in overflow-hidden rounded-xl border border-line bg-surface shadow-card',
        className
      )}>
      {header}
  ...
  return (
    <div className={clsx('flex flex-wrap items-start justify-between gap-x-4 gap-y-3', className)}>
      <div className="min-w-0">
        <h1 className="flex items-baseline gap-2 text-2xl font-bold leading-tight tracking-[-0.024em] text-primary">
          <span className="min-w-0 truncate">{title}</span>
          {counter != null && (
        ...
        {description && (
          <p className="mt-1.5 max-w-[68ch] text-sm leading-6 text-secondary">{description}</p>
        )}
      </div>
      {actions && <div className="flex flex-none flex-wrap items-center gap-2">{actions}</div>}
    </div>
