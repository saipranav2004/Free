...
export function MenuButton({ icon: Icon, children, open, count, className }) {
  return (
    <span
      className={clsx(
        'inline-flex h-9 flex-none cursor-pointer select-none items-center gap-2 rounded-lg border px-3.5 text-sm font-medium transition-colors duration-100',
        open
          ? 'border-line-strong bg-hover text-primary'
          : 'border-line-strong bg-surface text-primary hover:bg-hover',
        className
      )}
    >
