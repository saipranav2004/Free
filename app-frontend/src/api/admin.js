...
// The dashboard's charts as COUNTS, computed in the database, instead of rows
// walked 200 at a time and totalled in the browser.
//
// The row-walking path is capped at 5,000 events because past that the page is
// waiting on round trips and megabytes. This has no such ceiling: the response
// is a few hundred bytes whether it describes five events or five million, so
// the numbers cover everything in range rather than the most recent slice.
//
// tz is not optional in practice. Rows are stored in UTC, and "which hour did
// this happen in" only has an answer relative to where the reader is sitting.
// Without it an evening event lands on the next day and the heatmap is quietly
// wrong, so the browser's own zone is sent by default.
export async function auditStats(params = {}, signal) {
  const tz =
    params.tz ||
    (typeof Intl !== 'undefined' ? Intl.DateTimeFormat().resolvedOptions().timeZone : '') ||
    'UTC'
  const { data } = await http.get('/api/v1/pam/admin/audit/stats', {
    params: { ...params, tz },
    signal,
  })
