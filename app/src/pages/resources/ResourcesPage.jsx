...
export default function ResourcesPage() {
  ...
  const onConnect = useCallback((r) => setConnectTarget(r), [])
  ...
  return (
    <Stack gap="lg">
      ...
      {/* The real connect flow. ConnectPanel fetches connect-info, surfaces the
          403 when the resource is JIT gated and the caller has no grant, and
          owns starting and ending the tracked session. */}
      <Drawer
        open={!!connectTarget}
        onClose={() => setConnectTarget(null)}
        title={connectTarget ? `Connect to ${connectTarget.name}` : 'Connect'}
        subtitle={
          connectTarget
            ? `${connectTarget.host}${connectTarget.port ? `:${connectTarget.port}` : ''}`
            : undefined
        }
        width="lg"
      >
        {connectTarget && (
          <div className="p-4">
            <ConnectPanel resourceId={connectTarget.id} />
          </div>
        )}
      </Drawer>
